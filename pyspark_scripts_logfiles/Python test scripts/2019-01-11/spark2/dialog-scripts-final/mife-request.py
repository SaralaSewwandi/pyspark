...
temp_df=request_rows.select(request_rows.cols[0].alias('0'),regexp_extract(request_rows.cols[1],r'-\d\d\d\d',0).alias('1'),request_rows.cols[2].alias('2'),regexp_extract(request_rows.cols[3],r'\d\d\d\d-\d\d-\d\d',0).alias('date'),regexp_extract(request_rows.cols[4],r'\d\d:\d\d:\d\d',0).alias('time'),request_rows.cols[16].alias('api_info'),request_rows.cols[21].alias('api_request_url'))

#create request data frame
request_df=temp_df.select(temp_df['0'],temp_df['1'],temp_df['2'],temp_df['date'],temp_df['time'],split(temp_df['api_info'],"""\|""")[0].alias('api_id'),split(temp_df['api_info'],"""\|""")[1].alias('api_name'),split(temp_df['api_request_url'],",")[0].alias('request_url'))
