...
api_info=request_rows.select(request_rows.cols[16].alias('api_info'))


api_info_rows=api_info.select(split(api_info['api_info'],"""\|""").alias('api_info_cols'))

api_id=api_info_rows.select(api_info_rows.api_info_cols[0].alias('api_id'))

api_name=api_info_rows.select(api_info_rows.api_info_cols[1].alias('api_name'))



api_request_url=request_rows.select(request_rows.cols[21].alias('api_request_url'))

#create final data frame
df=request_rows.select(regexp_extract(request_rows.cols[3],r'\d\d\d\d-\d\d-\d\d',0).alias('data'),regexp_extract(request_rows.cols[4],r'\d\d:\d\d:\d\d',0).alias('time'))
